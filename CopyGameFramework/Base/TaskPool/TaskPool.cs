...
namespace CopyGameFramework
{
    ...
    /// 任务池
    /// </summary>
    /// <typeparam name="T">任务池</typeparam>
    internal sealed class TaskPool<T> where T : ITask
    {
        ...
        public void Update(float elapseSeconds, float realElapseSeconds)
        {
            ...
            //从未使用的任务代理中取出一个加入到工作中的任务代理中，
            //开始等待任务中的第一个，如果完成，停止处理中的任务重置
            while (FreeAgentCount > 0 && WaitingTaskCount > 0)
            {
                ...
                {
                    agent.Reset();
                    m_FreeAgents.Push(agent);           //重新放回未使用的任务代理中
                    m_WorkingAgents.Remove(agentNode); //移除出工作中代理队列
                }
            }
        }
