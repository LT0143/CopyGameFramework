   ...
   public static class ReferencePool
    {
        ...
        public static T Acquire<T>() where T : class, IReference, new()
        {
            lock (s_ReferencePool)
            {
                Queue<IReference> referencesPool = GetReferencePool(typeof(T).FullName);
                if (referencesPool.Count > 0)
                {
                    return (T)referencesPool.Dequeue();
                }
            }
            return new T();
        }
        ...
        public static IReference Acquire(Type referenceType)
        {
            ...
            lock (s_ReferencePool)
            {
                Queue<IReference> referencesPool = GetReferencePool(referenceType.FullName);
                if (referencesPool.Count > 0)
                {
                    return referencesPool.Dequeue();
                }
            }
        ...
        {
            Queue<IReference> referencesPool = null;
            if (!s_ReferencePool.TryGetValue(fullName,out referencesPool))
            {
                referencesPool = new Queue<IReference>();
                s_ReferencePool.Add(fullName, referencesPool);
            }
            return referencesPool;
        }
    }
}
