...
namespace CopyGameFramework
{
    ...
    internal sealed partial class EventPool<T> where T: BaseEventArgs
    {
        ...
        /// <param name="realElapsSeconds">真实流逝时间，以秒为单位</param>
        ...
        public void Subscribe(int id, EventHandler<T> handler)
        {
            ...
            {
                eventHandler += handler;   //已经存在事件类型则添加处理函数
        ...
        /// <summary>
        /// 抛出事件，这个操作是线程安全的，即使不在主线程中抛出，也可保证在主线程中回调事件
        /// 处理函数，但事件会在抛出后的下一帧分发。
        /// </summary>
        ...
        private void HandleEvent(object sender, T e)
        {
            ...
            if (m_EventHandlers.TryGetValue(e.Id,out handler))
            {
                if (handler != null)
                {
                    handler(sender, e);
